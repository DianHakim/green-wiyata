                        ...
                        <div class="col-md-4 mb-3">
                            <div class="card h-100">
                                ...
                                <div class="card-footer d-flex justify-content-between">
                                    <a href="{{ route('posts.show', $post) }}" class="btn btn-info btn-sm">Detail</a>
                                    <a href="{{ route('posts.edit', $post) }}" class="btn btn-warning btn-sm">Edit</a>

                                    <form action="{{ route('posts.delete', $post) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm" 
                                                onclick="return confirm('Yakin hapus post ini?')">Hapus</button>
                                    </form>
                                </div>
                            </div>
                        </div>
